...
export function BlackHoleControls({ simulation, onChange, onReset }: { simulation: Simulation; onChange: (next: Simulation) => void; onReset: () => void }) {
  ...
  return <aside className={`bh-controls ${open ? "is-open" : ""}`} aria-label="Black hole simulation controls"><div className="bh-controls-head"><p><span className="bh-live-dot" />Simulation controls</p><div><button type="button" className="bh-controls-toggle" aria-expanded={open} onClick={() => setOpen((value) => !value)}>{open ? "Close" : "Controls"}</button><button type="button" onClick={onReset}>Reset</button></div></div><div className="bh-controls-body">{sliders.map(([key, label, unit]) => <label key={key} className="bh-slider"><span>{label}<b>{simulation[key]}{unit === "solar masses" ? " M☉" : unit}</b></span><input type="range" min="0" max="100" value={simulation[key] as number} onChange={(event) => onChange({ ...simulation, [key]: Number(event.target.value) })} /></label>)}<div className="bh-toggles">{toggles.map(([key, label]) => <label key={key}><input type="checkbox" checked={simulation[key] as boolean} onChange={(event) => onChange({ ...simulation, [key]: event.target.checked })} /><span>{label}</span></label>)}</div></div></aside>;
}
